 ...
 public:
  ...
  void runOnOperation() override {
    ...
    if (blob_or.ok()) {
      ...
      auto debug_flag = std::getenv("AUTO_FUSION_DEBUG");
      if (debug_flag == "true" || debug_flag == "1") {
        std::ofstream outfile("cubin", std::ios::binary);
        if (!outfile) {
          llvm::errs() << "Error opening file";
          std::terminate();
        }
        outfile.write(blob_string.c_str(), blob_string.size());
        outfile.close();
      }
  ...
  static llvm::raw_fd_ostream& getSonyOs() {
    ...
    static llvm::raw_fd_ostream sonyOs("GpuKernelToBlobPass.mlir", errCode);
    if (errCode.value()) {
      std::terminate();
    }
  ...
  tensorflow::StatusOr<std::vector<uint8_t>> GetGpuBinaryBlob(
    ...
    auto gpu_module_name = gpu_module.getName();
    ...
    for (const std::string& arch_str : architectures_) {
      ...
      if (gpu_module_name == "predict_online_1_kernel") {
        std::lock_guard<std::mutex> lk(sony_mutex);
        sonyOs << "// Sony: Before CompileToPtx\n";
        sonyOs << *llvm_module_copy.get() << "\n";
        sonyOs << "// ==========================================================================================\n\n";
        sonyOs.flush();
      }
      ...
      if (gpu_module_name == "predict_online_1_kernel") {
        std::lock_guard<std::mutex> lk(sony_mutex);
        sonyOs << "// Sony: After CompileToPtx (PTX output)\n";
        sonyOs << ptx << "\n";
        sonyOs << "// ==========================================================================================\n\n";
        sonyOs.flush();
      }
      ...
      if (!is_compute_profile) {
        ...
        if (gpu_asm.ok()) {
          ...
          LOG(FATAL) << gpu_asm.status().error_message().c_str()
                     << "Falling back to compilation by driver.";
