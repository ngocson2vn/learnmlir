...
#define DEBUG_TYPE "common-passes"
...
struct LowerMemRefToLLVMFuncOpPattern : public mlir::OpConversionPattern<FunctionOpType> {  
  using OpConversionPattern<FunctionOpType>::OpConversionPattern;

  LogicalResult matchAndRewrite(FunctionOpType oldFunc, typename FunctionOpType::Adaptor adaptor, mlir::ConversionPatternRewriter &rewriter) const override {
...
struct LowerMemRefToLLVMReturnOpPattern : public OpConversionPattern<ReturnOpTy> {
  using OpConversionPattern<ReturnOpTy>::OpConversionPattern;

  LogicalResult matchAndRewrite(ReturnOpTy oldReturnOp, typename ReturnOpTy::Adaptor adaptor, ConversionPatternRewriter &rewriter) const override {
...
struct LowerMemRefToLLVMLaunchFuncOpPattern : public OpConversionPattern<gpu::LaunchFuncOp> {
  using OpConversionPattern::OpConversionPattern;

  LogicalResult matchAndRewrite(gpu::LaunchFuncOp oldLaunchOp, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
    llvm::outs() << "\nParent op:\n";
    llvm::outs() << *oldLaunchOp->getParentOp() << "\n\n";

    auto asyncToken = oldLaunchOp.getAsyncToken();
    Type asyncTokenType = asyncToken ? asyncToken.getType() : Type();
    auto clusterSize = oldLaunchOp.hasClusterSize() ? 
                       std::make_optional(oldLaunchOp.getClusterSizeOperandValues()) : 
                       std::nullopt;
    auto newLaunchOp = rewriter.create<gpu::LaunchFuncOp>(oldLaunchOp.getLoc(), 
                                                          oldLaunchOp.getKernel(),
                                                          oldLaunchOp.getGridSizeOperandValues(),
                                                          oldLaunchOp.getBlockSizeOperandValues(),
                                                          oldLaunchOp.getDynamicSharedMemorySize(),
                                                          adaptor.getKernelOperands(),
                                                          asyncTokenType,
                                                          oldLaunchOp.getAsyncDependencies(),
                                                          clusterSize);
    rewriter.replaceOp(oldLaunchOp, newLaunchOp);
...
class LowerMemRefToLLVMPass : public mlir::toy::impl::LowerMemRefToLLVMPassBase<LowerMemRefToLLVMPass> {
  ...
  void runOnOperation() override {
    ...
    patterns.add<
      // LowerMemRefToLLVMFuncOpPattern<gpu::GPUFuncOp>,
      // LowerMemRefToLLVMReturnOpPattern<gpu::ReturnOp>,
      LowerMemRefToLLVMFuncOpPattern<func::FuncOp>,
      LowerMemRefToLLVMLoadOpPattern,
      LowerMemRefToLLVMStoreOpPattern,
      LowerMemRefToLLVMReturnOpPattern<func::ReturnOp>
    ...
    //   for (const auto& kernelArg : launchOp.getKernelOperands()) {
    //     if (!typeConverter.isLegal(kernelArg.getType())) {
    //       return false;
    //     }
    //   }
    ...
    // patterns.clear();
    // patterns.add<
    //   LowerMemRefToLLVMLaunchFuncOpPattern
    // >(typeConverter, &getContext());
    // // Apply partial convertion
    // if (failed(applyPartialConversion(module, target, std::move(patterns), config))) {
    //   signalPassFailure();
